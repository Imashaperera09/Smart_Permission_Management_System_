...
export default function ManagerApprovalList() {
    ...
    const fetchPendingRequests = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/LeaveRequest/all`)
            if (response.ok) {
    ...
    const handleAction = async (id, action) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/LeaveRequest/${action}/${id}`, {
                method: 'POST'
            })
            if (response.ok) {
                fetchPendingRequests()
                                ...
                                <tr key={req.id} className="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col gap-2">
                                            <p className="text-sm font-medium text-slate-600 max-w-xs italic">"{req.reason || 'No reason provided'}"</p>
                                            {req.medicalUrl && (
                                                <a
                                                    href={req.medicalUrl}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.15em] text-rose-600 bg-rose-50 w-fit px-3 py-1 rounded-lg border border-rose-100 hover:bg-rose-100 transition-all"
                                                >
                                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a11 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                                    Medical Document
                                                </a>
                                            )}
                                        </div>
